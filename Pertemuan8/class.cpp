...
class Mahasiswa {
    //? Access Modifier
    //private tidak bisa dilihat selain user
    private:
        string nama, nim;
        float nilai;

    //? Access Modifier
    //public bisa dilihat oleh semua org 
    public:
...
int main() {
    ...
    cout << siswa1.getName() << endl;//mencari function getName
    ...
    for(int i = 0; i < n; i++) {
        ...
        Mahasiswa mhs(nama, nim, nilai); //deklarasi Mahasiswa menjadi mhs dengan nama, nim, nilai
        mahasiswa.push_back(mhs);
    }

    for(int i = 0; i < n; i++) {
        cout << "Mahasiswa " << i + 1 << endl;
        mahasiswa[i].display(); //mencari function display
    }
